...
// Last Edit: 2017-5-14
// Correction EEPROM.commit(); vs. EEPROM.h conflict

// Last Edit: 2017-6-05
// Increase handleGetEEPROM transmmission EEPROM mode
...
// Version
String FW_Version = "Citro Firmware Version 5.40 (2018/04/10)";
String Thanks_1 = "Based on Miniplan V6 by Roy Chen and Mason Chen";
String Thanks_2 = "";
...
void Servo_PROGRAM_Run(int iMatrix[][ALLMATRIX],  int iSteps)
{
  ...
  for ( int MainLoopIndex = 0; MainLoopIndex < iSteps && Servo_cancel_motion != 1; MainLoopIndex++)  // iSteps - Steps main loop
  {
    // InterTotalTime - Total time of this step
    int InterTotalTime = iMatrix [ MainLoopIndex ] [ ALLMATRIX - 1 ] + (int8_t)EEPROM.read(ALLMATRIX - 1); // iMatrix is the action stored on EEPROM and (int8_t)EPROM.read are the offsets

    // InterDelayCounter - this step basically delays
    int InterDelayCounter = InterTotalTime / BASEDELAYTIME;

    // Inner difference loop
    for ( int InterStepLoop = 0; InterStepLoop < InterDelayCounter && Servo_cancel_motion != 1; InterStepLoop++)
    {
      Serial.print(MainLoopIndex);
      Serial.print(" ");
      Serial.print(InterStepLoop);
      Serial.print(" ");
      Serial.print(InterDelayCounter);
      Serial.println(" ");

      for (int ServoIndex = 0; ServoIndex < ALLSERVOS; ServoIndex++)  // Motor main loop
      {

        INT_TEMP_A = Running_Servo_POS[ServoIndex];                                                    // Motor current location
        INT_TEMP_B = iMatrix[MainLoopIndex][ServoIndex] + (int8_t)EEPROM.read(ServoIndex);    // Motor target position

        if (INT_TEMP_A == INT_TEMP_B)         // The motor value is unchanged
        {
        ...
        {
          // PWM inner difference = map( Accumulated time execution, Starting time, End Time, Inner difference start value, Internal difference Maximum )
        ...
        else if (INT_TEMP_A < INT_TEMP_B)   // the motor value is increased
        {
          // PWM Inner difference = map( Accumulated time execution, Starting time, End Time, Inner difference start value, Internal difference Maximum )
    ...
    // Backup the current motor value
    for ( int Index = 0; Index < ALLMATRIX; Index++)
    {
...
void handleSave()
{
  ...
  if (keyInt == 100)
  {
    ...
    writeKeyValue(19, 0);
    writeKeyValue(20, 0);
    //writeKeyValue(21, 0); // Set_PWM_Freq - Do NOT Clear
    //writeKeyValue(22, 0); // Set_Voltage - Do NOT Clear
  }
  else
  {
    // Confirmation data is between -125 ~ 125
    if (valueInt >= -125 && valueInt <= 125)
    {
      // Store corrections
      writeKeyValue(keyInt, valueInt);

      // if Set_PWM_Freq has been updated immediately
      if (keyInt == 21) {
        Set_PWM_Freq = PWM_Frequency + valueInt;
        pwm.setPWMFreq(Set_PWM_Freq);
      }

      // If Set_Voltage has been updated immediately
      if (keyInt == 22) {
...
/*============================================================================
  Motor Offset Correction Screen
  ============================================================================*/
void handleSetting()
{
...
bool handleFileRead(String path) { // send the right file to the client (if it exists)
  ...
  if (path.endsWith("/zero")) path += ".html";
  if (path.endsWith("/editor")) path += ".html";
...
void setup(void) {
  ...
  //initialize the MPU-6050. By default it sleeps so we have to wake it up
  Wire.beginTransmission(gyro_address);                                     //Start communication with the gyro address
...
//    }
  ...
  for(receive_counter = 0; receive_counter < 500; receive_counter++){       //Create 500 loops
    if(receive_counter % 15 == 0)digitalWrite(15, !digitalRead(15));        //Change the state of the LED every 15 loops to make the LED blink fast
  ...
  //display.println("P= " + gyro_pitch_calibration_value);
  //display.println("Y= " + gyro_yaw_calibration_value);
  //display.display();
  //OLED_Display_Index = 24;
...
void loop(void) {
  ...
  display.println("P= " + String(gyro_pitch_data_raw));
  display.println("Y= " + String(gyro_yaw_data_raw));
  //display.display();
  ...
  if (Servo_PROGRAM_Stack >= 1 && Input_Voltage_Low == 0 )
  {
    ...
    switch (Servo_PROGRAM_Stack)
    {
      ...
      case 7:  // Clapping Action
        ...
        Servo_PROGRAM_Center();
        break;
      case 8:  // Monster Watch Action
        Servo_PROGRAM_Run(Servo_Prg_38, Servo_Prg_38_Step);
        Servo_PROGRAM_Center();
        break;
      case 9:  // Push ups action
        ...
        Servo_PROGRAM_Center();
        break;
      case 10: // Tai Chi Action
        Servo_PROGRAM_Run(Servo_Prg_40, Servo_Prg_40_Step);
        Servo_PROGRAM_Center();
        break;
      case 99:  // AUTO DEMO of Actions
        Servo_PROGRAM_Run(Servo_Prg_32, Servo_Prg_32_Step);
        Servo_PROGRAM_Center();
        delay(500);
        Servo_PROGRAM_Run(Servo_Prg_36, Servo_Prg_36_Step);
  ...
  if (Voltage_Conter >= 30000)
  {
    ...
    // Automatically Correct Voltage Error
    if (Engineering_Model == 1)
    {
      if (Voltage >= 601) {
    ...
    else
    {
      ...
      {
        // Low Voltage - Prohibit Action Movement
        //Input_Voltage_Low = 1;
      }
    }
